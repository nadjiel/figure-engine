...
describe("Sprite class", () => {
  ...
  it("Should instantiate properly without configurations", () => {
    const initialMargins = 0;
    const initialGaps = 0;
    const initialIncludedFrames = [0];
    const initialFrameId = 0;

    expect(sprite.getImage()).toBe(imageResource.get());
    expect(sprite.getWidth()).toBe(imageResource.get().width);
    expect(sprite.getHeight()).toBe(imageResource.get().height);
    ...
    expect(sprite.getFrameId()).toBe(initialFrameId);
    expect(sprite.getFrame()).toBeInstanceOf(HTMLImageElement);
  });
